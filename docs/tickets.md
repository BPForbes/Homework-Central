...
| Local run-dev | `scripts/run-dev.* --stripped` / `-Stripped` sets `HC_DEV_STRIPPED=1`: leftover Queued/Running sessions are paused via `StopTrainingSessionAsync`, and neural warmup/refresh/in-process worker do not start. Default and stripped `run-dev` both start Postgres with `start_dev_stack_postgres_container` / `Start-DevStackPostgresContainer` and background the existing FCaptcha helper so a cold image build is not joined before `/healthz`. `run-dev` waits for in-container `pg_isready` and proof that `127.0.0.1:<POSTGRES_HOST_PORT>` reaches Postgres before the API. `PostgresHostCheck` exits `0` when `homework_central_master` answers, `3` when a server answers but hands back no usable `homework_central_master` — normally a fresh volume without that database, and also the fallback for any unmapped `SqlState` — `4` when the server is not accepting sessions yet, `5` when a server rejects the dev credentials (`28P01`, so the volume was initialised with a different password), and `1` when nothing answers; readiness accepts `0`, `3` or `5` because master creation and volume reset run after the wait, and an in-container-ready Postgres that the published port cannot reach goes to `repair_postgres_host_reachability` / `Repair-PostgresHostReachability` instead of aborting. Exit `5` is the only signal for a mismatched password, because initdb writes `host all all 127.0.0.1/32 trust` ahead of the image's `scram-sha-256` rule and an in-container `psql` probe therefore authenticates against no password at all; `reset_stale_postgres_volume` / `Reset-StalePostgresVolume` recreates the volume on it, but only when our own container publishes the port — a foreign Postgres there stays a port-relocation case, and `repair_postgres_host_reachability` / `Repair-PostgresHostReachability` defers to the reset rather than relocating off our own stale volume. `reset_postgres_volume` / `Reset-PostgresVolume` removes the Postgres container plus the volume mounted at its data directory, read off the container, so `llmdata`, `uploads` and `miniodata` survive; `scripts/reset-dev-db.*` stays the way to ask for `docker compose down -v`. `start-api-dev`'s own wait has no repair behind it, so it logs the rejection detail when it clears on `3` or `5`. Development keeps retrying unreachable Postgres instead of stopping the host after ten refused connections. `run-dev` calls `init_dev_stack_state` / `Initialize-DevStackState` *before* that wait, because leftover `.hc-dev-stack.state` used to look like a previous session and stop the container the wait had just cleared; `scripts/reset-dev-db.*` now removes the state file with the volume. `start-api-dev` uses the same postgres-first helper even when `run-dev` already registered the stack, so a published port that died after the parent wait is started again instead of leaving the API to retry a refused connection. Local Development marks `/healthz` ready after auth seed; Production keeps catalog seed before Ready. |
...
| [backend/HomeworkCentral.Api/Assessment/ChatMonitoringNeuralModelWarmupService.cs](../backend/HomeworkCentral.Api/Assessment/ChatMonitoringNeuralModelWarmupService.cs) | BackgroundService warmup: capped silent Train replay of approved examples after Kestrel listens (avoids Full-trace OOM on host start). |
