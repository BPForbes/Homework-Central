...
export function ReplayViewer({ replay }: { replay: NeuralNetReplay }) {
  ...
  const intermediate = intermediateLayerIds.flatMap(layerId => replay.topology.nodes.filter(node => node.layerId === layerId))
  ...
  const layerX = (layerId: string): number => 130 + layerIndex(layerId) * (1180 / Math.max(1, layerIds.length - 1))
  const nodesInLayer = (layerId: string): ReplayNode[] => layerId === 'input' ? visibleInputs : replay.topology.nodes.filter(node => node.layerId === layerId)
  const nodeAt = (node: ReplayNode): { x: number; y: number } => {
    ...
    return { x: layerX(node.layerId), y: 62 + index * (550 / Math.max(1, layerNodes.length - 1)) }
  }
  ...
  return <section className="sm-panel neural-graph-panel">
    ...
    <svg className="neural-graph neural-graph--replay" viewBox="0 0 1440 680" role="img" aria-label="Recorded neural network topology">
      {layerIds.map(layerId => <text key={`label-${layerId}`} x={layerX(layerId)} y="28" textAnchor="middle" className="neural-layer-label">{layerLabel(layerId)}</text>)}
      ...
      {detail === 0
        ? layerIds.map(layerId => { const layerNodes = replay.topology.nodes.filter(node => node.layerId === layerId); return <g key={`cluster-${layerId}`}><circle cx={layerX(layerId)} cy="340" r="58" className="neural-node" /><text x={layerX(layerId)} y="346" textAnchor="middle" className="neural-node-text">{layerNodes.length} nodes</text></g> })
        : nodes.filter(node => node.layerId === 'input').map(node => { const point = nodeAt(node); return <circle key={node.nodeId} cx={point.x} cy={point.y} r="6" className="neural-node" /> })}
      {intermediate.map(node => { const point = nodeAt(node); return <g key={node.nodeId} className="neural-node-group" onClick={() => setSelected(node)}><circle cx={point.x} cy={point.y} r="15" className={`neural-node ${selected?.index === node.index ? 'neural-node--selected' : ''}`} /><text x={point.x} y={point.y + 5} textAnchor="middle" className="neural-node-text">{node.label.replace(/^.*?\s/, '')}</text></g> })}
      {output.map(node => { const point = nodeAt(node); return <g key={node.nodeId} className="neural-node-group" onClick={() => setSelected(node)}><rect x={point.x - 70} y={point.y - 22} width="140" height="44" rx="12" className={`neural-node ${selected?.index === node.index ? 'neural-node--selected' : ''}`} /><text x={point.x} y={point.y + 5} textAnchor="middle" className="neural-node-text">{node.label}</text></g> })}
    </svg>
